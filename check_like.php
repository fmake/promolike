	...
	switch ($item['id_place']){
		case '2':
			...
			elseif($res->response){	
				...
				//printAr($res);
				$is_like_wall = $promoLike->checkLikeWallVK($array_users_wall_vk[$item[id_user_place]]->response,$item['like_text'],$item['url']);
			...
			break;
		case '3':
			echo("проверили лайк в twitter.com с Id = {$item[$promoLike->idField]}<br/>");	
			echo("<br />--------------------------------------------------<br />");			
			break;	
		}		
}
